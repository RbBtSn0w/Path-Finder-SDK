...
@interface NTSizeCalculatorTree (Protocols) <NTOperationDelegateProtocol, NTMessageProxyProtocol>
@end

@interface NTSizeCalculatorTree ()
@property (nonatomic, retain) NTMessageProxy* proxy;
@property (retain) NTFSEventClient* eventClient;
@end
...
+ (NTSizeCalculatorTree*)treeForVolume:(NTVolume*)theVolume;
{
	...
	// start eventClient
	result.eventClient = [NTFSEventClient client:result.proxy folder:[theVolume mountPoint] includeSubfolders:YES];
...
- (void)schedulePendingFoldersOperation;
{
	if (!self.sentDelayedPerformPendingFolders)
	{
		...
		[self performDelayedSelector:@selector(schedulePendingFoldersOperationAterDelay) withObject:nil delay:.25];
	}
}
...
- (void)schedulePendingRefreshOperation;
{
	if (!self.sentDelayedPerformPendingRefresh)
	{
		...
		[self performDelayedSelector:@selector(schedulePendingRefreshOperationAterDelay) withObject:nil delay:3];
	}
}
...
{		
	NSArray* theMessages = [inMessage objectForKey:kFSEventClient_messagesKey];
	for (NTFileSystemEventMessage *theMessage in theMessages)
	{
		NTFileSystemEventMessage* existingMessage = [self.pendingRefreshMessages objectForKey:theMessage.path];
		
		// item already exists, add the paths and the rescan flag
		if (existingMessage)
			[existingMessage updateMessage:theMessage.rescanSubdirectories];
		else
			[self.pendingRefreshMessages setObject:theMessage forKey:theMessage.path];
	}

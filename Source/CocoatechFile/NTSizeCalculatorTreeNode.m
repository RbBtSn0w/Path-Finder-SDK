...
- (void)markDirty:(BOOL)mustRescanChildren;
{
	...
	{
		NSLogNULL(@"DIRTY: %@", [self description]);
	...
	if (!self.sizeIsValid)
	{
		...
		if (!operation.isCancelled)
		{
			if (result)
			{
				...
				NSLogNULL(@"-[%@ %@] result nil? (!operation.isCancelled && !result)", [self className], NSStringFromSelector(_cmd));
		}
	}
}
...
- (NTFSSize*)doCombineSizeWithUpdatedChildren:(NSOperation*)operation;
{
	...
	if (!operation.isCancelled)
	{
		...
		if (self.size)
		{
			...
			NSLogNULL(@"-[%@ %@] size nil?", [self className], NSStringFromSelector(_cmd));
	}
...
- (NSArray*)childSizesUsingPreviousSize:(NTFSSize*)previousSize;
{
	...
	if (previousSize)
	{
		...
		if (!childNodeIDs.count)
			NSLogNULL(@"-[%@ %@] no children? %@", [self className], NSStringFromSelector(_cmd), treePath);
	}
	...
	{
		if (!node.size)
			NSLogNULL(@"-[%@ %@] node size is nil? %@", [self className], NSStringFromSelector(_cmd), node.treePath);
		else
		{
			if (childNodeIDs)
			{
	...
	if (self.needToVerify)
	{
		...
		else
		{
			...
			if ([self.desc stillExists])
			{
				...
				NSLogNULL(@"-[%@ %@] node bad, readd to tree: %@", [self className], NSStringFromSelector(_cmd), self.treePath);
			}
			else
			{
				[deletedNodes addObject:self];
				NSLogNULL(@"-[%@ %@] node bad, deleted: %@", [self className], NSStringFromSelector(_cmd), self.treePath);
			}
		}
	}

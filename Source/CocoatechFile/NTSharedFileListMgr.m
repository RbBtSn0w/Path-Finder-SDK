...
#define kNTIgnoreRefreshTimeInterval 1.0

@interface NTSharedFileListMgr ()
@property (nonatomic, assign) LSSharedFileListRef favoriteVolumes;
@property (nonatomic, assign) LSSharedFileListRef favoriteFiles;
@property (nonatomic, assign) LSSharedFileListRef recentApplications;
@property (nonatomic, assign) LSSharedFileListRef recentDocuments;
@property (nonatomic, assign) LSSharedFileListRef recentServers;
@property (nonatomic, assign) LSSharedFileListRef loginApplications;
@property (nonatomic, retain) NSMutableSet* notificationKeys;
@property (nonatomic, assign) BOOL sentDelayedNotification;
@end
...
{
	CFNumberRef numberRef = LSSharedFileListCopyProperty(self.recentDocuments , kLSSharedFileListRecentItemsMaxAmount);
	NSUInteger result = 0;
	
	if (numberRef)
	{
		result = [(NSNumber*)numberRef unsignedIntegerValue];
...
{
	// don't allow setting to zero or over 100
	if (num == 0)
		num = 1;
	if (num > 100)
		num = 100;
	
	LSSharedFileListSetProperty(self.recentDocuments,
								kLSSharedFileListRecentItemsMaxAmount,
								(CFTypeRef) [NSNumber numberWithUnsignedInteger:num]);
}

- (NSUInteger)numberOfRecentApplications;
{
	CFNumberRef numberRef = LSSharedFileListCopyProperty(self.recentApplications , kLSSharedFileListRecentItemsMaxAmount);
	NSUInteger result = 0;
...
- (void)setNumberOfRecentApplications:(NSUInteger)num;
{
	...
	LSSharedFileListSetProperty(self.recentApplications,
								kLSSharedFileListRecentItemsMaxAmount,
								(CFTypeRef) [NSNumber numberWithUnsignedInteger:num]);
}
...
{
	LSSharedFileListRef theList = [self listForID:theListID];
	NSArray* theListItems = [self itemsForList:theList];

	for (NTSharedFileListItem* theItem in theListItems)
	{
		if ([theUniqueID isEqual:theItem.uniqueID])
			return theItem;
	}
...
- (NSArray*)itemsForList:(LSSharedFileListRef)theList; 
{
	...
	if (theList)
	{
		...
		if (snapshot)
		{
			NSString* listID = [self listIDForList:theList];
			
			for (id itemRef in (NSArray*)snapshot)
				[result addObject:[NTSharedFileListItem item:(LSSharedFileListItemRef)itemRef listID:listID]];
		...
		if ([theDesc isValid])
		{
			...
			{	
				...
				if (result)
					...
					NSLog(@"LSSharedFileListInsertItemURL failed? : %@", [theDesc URL]);
			}
		}
	}
}
...
@interface NTSharedFileListMgr (MenusPrivate)
- (void)numberOfRecentApplicationsAction:(NSMenuItem*)sender;
- (NSMenuItem*)numberOfRecentApplicationsMenuItem:(NSUInteger)theNumber checkItem:(BOOL)checkItem fontSize:(NSUInteger)fontSize;
- (NSMenu*)numberOfRecentApplicationsMenu:(NSUInteger)fontSize;
- (void)numberOfRecentDocumentsAction:(NSMenuItem*)sender;
- (NSMenuItem*)numberOfRecentDocumentsMenuItem:(NSUInteger)theNumber checkItem:(BOOL)checkItem fontSize:(NSUInteger)fontSize;
- (NSMenu*)numberOfRecentDocumentsMenu:(NSUInteger)fontSize;
@end
...
//		[menuItem setEnabled:NO];		
		...
		[menuItem setSubmenu:[self numberOfRecentDocumentsMenu:fontSize]];
		...
		for (NTSharedFileListItem* theItem in theItems)
		{
			...
			// set icon
			NSImage* theIcon = [theItem imageWithSize:iconSize];
			if (!theIcon)
				theIcon = [[NTMS documentIcon] imageWithSetSize:iconSize];
...
{    
	...
	MENU_DISABLE(resultMenu);
	{
		...
		for (NTSharedFileListItem* theItem in theItems)
		{
			...
			NSImage* theIcon = [theItem imageWithSize:iconSize];
			if (!theIcon)
				theIcon = [[NTMS fileServerIcon] imageWithSetSize:iconSize];
			[menuItem setImage:theIcon];
...
//		[menuItem setEnabled:NO];
		...
		[menuItem setFontSize:fontSize color:nil];
		[menuItem setEnabled:YES];
		[menuItem setSubmenu:[self numberOfRecentApplicationsMenu:fontSize]];
...
@implementation NTSharedFileListMgr (MenusPrivate)

- (void)numberOfRecentApplicationsAction:(NSMenuItem*)sender;
{
	[[NTSharedFileListMgr sharedInstance] setNumberOfRecentApplications:[sender tag]];
}

- (NSMenuItem*)numberOfRecentApplicationsMenuItem:(NSUInteger)theNumber checkItem:(BOOL)checkItem fontSize:(NSUInteger)fontSize;
{
	NSMenuItem* menuItem = [[[NSMenuItem alloc] initWithTitle:[NSString stringWithFormat:@"%d", theNumber] action:@selector(numberOfRecentApplicationsAction:) keyEquivalent:@""] autorelease];
	[menuItem setFontSize:fontSize color:nil];
	[menuItem setEnabled:YES];
	[menuItem setTarget:self];
	[menuItem setTag:theNumber];
	[menuItem setState:checkItem ? NSOnState:NSOffState];
...
- (NSMenu*)numberOfRecentApplicationsMenu:(NSUInteger)fontSize;
{
	...
	NSUInteger numRecents = [[NTSharedFileListMgr sharedInstance] numberOfRecentApplications];
	
	for (NSUInteger i=10; i<=50;i+=5)
		[theMenu addItem:[self numberOfRecentApplicationsMenuItem:i checkItem:(i == numRecents) fontSize:fontSize]];
...
- (void)numberOfRecentDocumentsAction:(NSMenuItem*)sender;
{
	[[NTSharedFileListMgr sharedInstance] setNumberOfRecentDocuments:[sender tag]];
}

- (NSMenuItem*)numberOfRecentDocumentsMenuItem:(NSUInteger)theNumber checkItem:(BOOL)checkItem fontSize:(NSUInteger)fontSize;
{
	NSMenuItem* menuItem = [[[NSMenuItem alloc] initWithTitle:[NSString stringWithFormat:@"%d", theNumber] action:@selector(numberOfRecentDocumentsAction:) keyEquivalent:@""] autorelease];
	[menuItem setFontSize:fontSize color:nil];
	[menuItem setEnabled:YES];
	[menuItem setTarget:self];
	[menuItem setTag:theNumber];
...
- (NSMenu*)numberOfRecentDocumentsMenu:(NSUInteger)fontSize;
{
	...
	NSUInteger numRecents = [[NTSharedFileListMgr sharedInstance] numberOfRecentDocuments];
	
	for (NSUInteger i=10; i<=50;i+=5)
		[theMenu addItem:[self numberOfRecentDocumentsMenuItem:i checkItem:(i == numRecents) fontSize:fontSize]];

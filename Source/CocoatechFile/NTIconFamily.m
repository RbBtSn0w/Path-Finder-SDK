...
{
	BOOL result = [[NSWorkspace sharedWorkspace] setIcon:[self image] forFile:[desc path] options:0];
...
- (BOOL)setAsCustomIconForDirectory:(NTFileDesc*)desc;
{
    if ([desc isValid] && [desc isDirectory])
    {
        if ([desc isVolume])
			...
			return [self setAsCustomIconForFile:desc];
    }
...
- (void)buildWithImage:(NSImage*)image;
{
    ...
    bitmap = [image bitmapImageRepForSize:512];
    if (bitmap) 
        [self setIconFamilyElement:kIconServices512PixelDataARGB fromBitmapImageRep:bitmap];

	bitmap = [image bitmapImageRepForSize:256];
    if (bitmap) 
        [self setIconFamilyElement:kIconServices256PixelDataARGB fromBitmapImageRep:bitmap];
...
- (BOOL)setIconFamilyElement:(OSType)elementType fromBitmapImageRep:(NSBitmapImageRep*)bitmapImageRep
{
    ...
    switch (elementType) {
		...
		case kIconServices512PixelDataARGB:
			hRawData = [NTIconFamily get32BitDataFromBitmapImageRep:bitmapImageRep requiredPixelSize:512];
			break;
			
			// 'ic08' 256x256 32-bit ARGB image
		case kIconServices256PixelDataARGB:
			hRawData = [NTIconFamily get32BitDataFromBitmapImageRep:bitmapImageRep requiredPixelSize:256];
			break;

...
- (BOOL)isDOCX;
{
    if (!_flags._isDOCX_initialized)
    {
        BOOL result = NO;
        
        if (_desc && ![_desc isDirectory] && ![_desc isAlias])
			result = UTTypeConformsTo ((CFStringRef) [_desc uniformTypeID], (CFStringRef)@"org.openxmlformats.wordprocessingml.document");
        
        _flags._isDOCX = (result) ? 1:0;
        _flags._isDOCX_initialized = 1;
    }
    
    return (_flags._isDOCX == 1);
}
...
- (BOOL)isRTFD;
{
    if (!_flags._isRTFD_initialized)
    {
		...
		if (_desc && [_desc isDirectory] && ![_desc isAlias])
		{
			// for speed, check the extension, UTTypeConformsTo is slow
			NSString* ext = [_desc extension];
			
			result = [ext isEqualToStringCaseInsensitive:@"rtfd"];
			if (result)
				result = UTTypeConformsTo ((CFStringRef) [_desc uniformTypeID], kUTTypeRTFD); // not necessary, but can't hurt (except slowness)
		}
	...
	if (!_flags._isHelpViewerPackage_initialized)
    {
		...
		if (_desc && [_desc isPackage])
		{
			...
			if ([[_desc extension] isEqualToStringCaseInsensitive:@"help"])
			{
				...
				paths = [NSBundle pathsForResourcesOfType:@"html" inDirectory:[_desc path]];
				if ([paths count] == 1)
					result = YES;
...
- (BOOL)isImageOrMedia;
{
	...
	if ([self isImage] || [self isMovie] || [self isAudio])
		result = YES;

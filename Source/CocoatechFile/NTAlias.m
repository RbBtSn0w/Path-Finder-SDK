...
@interface NTAlias ()
@property (nonatomic, retain) NTFileDesc *desc_storage;
@end
...
{
    if (self.desc_storage && resolveCompletely)
    {
        ...
        else // else just do a double check to see if the file is still the same
        {
            ...
            if (![self.desc_storage stillExists] || [self.desc_storage hasBeenModified])
                self.desc_storage = nil;
        }
    }
    
    if (!self.desc_storage)
    {
        // resolve the alias and recreate the self.desc_storage
        [self initDesc:resolveCompletely];
    }

	NTFileDesc *result = self.desc_storage;
...
- (void)initDesc:(BOOL)resolveIfRequiresUI;  // call this to resolve all aliases (remote servers that may ask for password);
{
    if (!self.desc_storage)
    {
        ...
        if (aliasHandle)
        {
            ...
            self.desc_storage = [NTAlias resolveAlias:nil aliasHandle:aliasHandle resolveIfRequiresUI:resolveIfRequiresUI outAliasRequiresUIToResolve:&outAliasRequiresUIToResolve outWasChanged:&wasChanged];
...
- (void)initAlias;
{
    if (!_alias)
    {
        if (self.desc_storage && [self.desc_storage isValid])
        {
            AliasHandle aliasHandle = [NTAlias aliasHandleForDesc:self.desc_storage];
            if (aliasHandle)
                [self setAlias:[NSData dataWithCarbonHandle:(Handle)aliasHandle]];
        }
    }
...
+ (NTFileDesc*)resolveAlias:(NTFileDesc*)fromFile 
...
outAliasRequiresUIToResolve:(BOOL*)outAliasRequiresUIToResolve
			  outWasChanged:(BOOL*)outWasChanged;
{
    ...
    if (aliasHandle)
    {
        ...
        err = FSResolveAliasWithMountFlags([fromFile FSRefPtr], aliasHandle, &targetRef, &wasChanged, mountFlags);
...
// ### WARNING: this was causing endless loops.  FSResolveAliasFileWithMountFlags would modify the file and cause endless updates for Path Finder. Use at your own risk.  It is a rare issue.

...
+ (NTFileDesc*)makeFolder:(NSString*)path permissions:(unsigned)permissions;
{
	...
	if (created)
	{
		...
		NSLog(@"mkdir failed:(%d)%s path:%@", errno, strerror(errno), path);
		
    return result;
}

+ (NTFileDesc*)makeFile:(NSString*)path permissions:(unsigned)permissions;
{
    NTFileDesc *result=nil;
    ...
    if (fd != -1)
    {
		...
		NSLog(@"open(create) failed:(%d)%s path:%@", errno, strerror(errno), path);
		 ...
		 withName:(NSString*)name; 
{
	...
	if (fileTag)
		destPath = [destPath stringByAppendingFormat:@" %@", fileTag];
	
	// get a unique name
	destPath = [[NTFileNamingManager sharedInstance] uniqueName:destPath with:nil];

...
@interface NTFSRefObject ()
@property (assign, nonatomic) FSCatalogInfo *catalogInfo;
...
- (BOOL)checkHasBeenModified:(BOOL)networkVolume;
@end

@implementation NTFSRefObject

@synthesize referenceDate, nameWhenCreated, catalogInfo;

- (id)initWithRef:(const FSRef*)ref
	  catalogInfo:(FSCatalogInfo*)theCatalogInfo
			 ...
			 name:(NSString*)name;
{
	...
	self.catalogInfo = NSZoneCalloc(NSDefaultMallocZone(), 1, sizeof(FSCatalogInfo));
    if (theCatalogInfo)
    {
        *(self.catalogInfo) = *theCatalogInfo;
...
- (void)dealloc;
{
	if (self.catalogInfo)
	{
		NSZoneFree(NSDefaultMallocZone(), self.catalogInfo);
...
- (BOOL)isNameLocked;
{
    ...
    return (([self finderFlags] & kNameLocked) != 0);
}
...
- (BOOL)isLocked;
{
    ...
    return ((self.catalogInfo->nodeFlags & kFSNodeLockedMask) != 0);
}
...
- (UInt32)valence;
{
    ...
    return self.catalogInfo->valence;    
}
...
- (NSDate*)modificationDate;
{
    ...
    return [NSDate dateFromUTCDateTime:self.catalogInfo->contentModDate];
}
...
- (NSDate*)creationDate;
{
    ...
    return [NSDate dateFromUTCDateTime:self.catalogInfo->createDate];
}
...
- (NSDate*)attributeModificationDate;
{
    ...
    return [NSDate dateFromUTCDateTime:self.catalogInfo->attributeModDate];
}
...
- (NSDate*)accessDate;
{
    ...
    return [NSDate dateFromUTCDateTime:self.catalogInfo->accessDate];
}
...
- (const FileInfo*)fileInfo;
{
    ...
    return ((FileInfo*)self.catalogInfo->finderInfo);
}
...
- (FSVolumeRefNum)volumeRefNum;
{
    ...
    return self.catalogInfo->volume;
}
...
- (BOOL)isDirectory;
{
    ...
    return ((self.catalogInfo->nodeFlags & kFSNodeIsDirectoryMask) != 0);
}
...
- (unsigned)parentDirID;
{
	...
	return (self.catalogInfo->parentDirID);
}
...
- (BOOL)isOpen;
{
    ...
    return ((self.catalogInfo->nodeFlags & kFSNodeForkOpenMask) != 0);
}
...
- (BOOL)isVolume;
{
    ...
    if ([self isDirectory])
        return ([self parentDirID] == fsRtParID);  // could also compare UTI to public.volume
...
- (OSType)fileType;
{
    ...
    if ([self isFile])
    {
        ...
        return ((FileInfo*)self.catalogInfo->finderInfo)->fileType;
    }
...
- (OSType)fileCreator;
{
    ...
    if ([self isFile])
    {
        ...
        return ((FileInfo*)self.catalogInfo->finderInfo)->fileCreator;
    }
...
- (BOOL)isStationery;
{
	...
	// files only
	if ([self isFile])
		return (([self finderFlags] & kIsStationery) != 0);
...
- (BOOL)isBundleBitSet;
{
    ...
    return (([self finderFlags] & kHasBundle) != 0);
}
...
- (BOOL)isAliasBitSet;
{
    ...
    return (([self finderFlags] & kIsAlias) != 0);
}
...
- (int)fileLabel;
{
    ...
    return (([self finderFlags] & kColor) >> 1);
}
...
- (UInt16)finderFlags;
{
    ...
    return ((FileInfo*)self.catalogInfo->finderInfo)->finderFlags;
}
...
- (NSPoint)finderPosition;
{
    ...
    carbonPoint = ((FileInfo*)self.catalogInfo->finderInfo)->location;
...
- (BOOL)isInvisible;
{
    ...
    BOOL result = (([self finderFlags] & kIsInvisible) != 0);
...
- (BOOL)hasCustomIcon;
{
    ...
    return (([self finderFlags] & kHasCustomIcon) != 0);
}
...
- (UInt32)ownerID;
{
    ...
    permPtr = (FSPermissionInfo*) &(self.catalogInfo->permissions);
...
- (UInt32)nodeID;
{
    ...
    return self.catalogInfo->nodeID;
}
...
- (UInt64)dataLogicalSize;
{
	...
	return self.catalogInfo->dataLogicalSize;
}
...
- (UInt64)dataPhysicalSize;
{
	...
	return self.catalogInfo->dataPhysicalSize;	
}
...
- (UInt64)rsrcLogicalSize;
{
	...
	return self.catalogInfo->rsrcLogicalSize;		
}
...
- (UInt64)rsrcPhysicalSize;
{
	...
	return self.catalogInfo->rsrcPhysicalSize;	
}
...
- (UInt8)sharingFlags;
{
    ...
    return self.catalogInfo->sharingFlags;
}
...
- (BOOL)hasBeenModified:(BOOL)networkVolume;
{
	if (!mv_hasBeenModified)
		mv_hasBeenModified = [self checkHasBeenModified:networkVolume];
...
- (void)updateForFlags:(FSCatalogInfoBitmap)flags
{
    ...
    if ([self flagsNeedUpdate:flags])
    {
        ...
        FSGetCatalogInfo(&_ref, minimalBits, self.catalogInfo, NULL, NULL, NULL);
    }
}
...
- (NSString*)description;
{
    ...
    FSGetCatalogInfo(&_ref, kFSCatInfoGettableInfo, &theCatalogInfo, NULL, NULL, NULL);
    FSPermissionInfo* permPtr = (FSPermissionInfo*) &(theCatalogInfo.permissions);
    ...
    [result appendString:@"\n"];
    [result appendString:[NSString stringWithFormat:@"nodeID: %d", theCatalogInfo.nodeID]];
    [result appendString:@"\n"];
    [result appendString:[NSString stringWithFormat:@"nodeFlags: %d", theCatalogInfo.nodeFlags]];
    [result appendString:@"\n"];
    [result appendString:[NSString stringWithFormat:@"sharingFlags: %d", theCatalogInfo.sharingFlags]];
    [result appendString:@"\n"];
...
- (BOOL)checkHasBeenModified:(BOOL)networkVolume;
{
	...
	// finderFlags changes like labels don't always update the attribute mod date on network volumes, check that here
	if (!hasBeenModified && networkVolume)
	{
		UInt16 previousFinderFlags = [self finderFlags];
		
		[self clearFlags:kFSCatInfoFinderInfo];
		
		if (previousFinderFlags != [self finderFlags])
			hasBeenModified = YES;
	}

...
{
    [NTIconCompositor drawIconFor:desc inRect:rect selected:NO drawLabel:(BOOL)drawLabel flip:flip];
}

+ (void)drawIconFor:(NTFileDesc*)desc inRect:(NSRect)rect selected:(BOOL)selected drawLabel:(BOOL)drawLabel flip:(BOOL)flip;
{
    [NTIconCompositor drawIconForRef:[[desc icon] iconRef] inRect:rect label:(drawLabel) ? [desc label]:0 selected:selected flip:flip];
}

+ (void)drawIconForRef:(IconRef)iconRef inRect:(NSRect)rect label:(int)label selected:(BOOL)selected flip:(BOOL)flip;
{
    [NTIconCompositor drawIconForRef:iconRef inRect:rect label:label selected:selected flip:flip alpha:1.0];
}

+ (void)drawIconForRef:(IconRef)iconRef inRect:(NSRect)rect label:(int)label selected:(BOOL)selected flip:(BOOL)flip alpha:(float)alpha;
{
	[NTIconCompositor drawIconForRef:iconRef inRect:rect label:label selected:selected flip:flip alpha:alpha alignment:kAlignAbsoluteCenter];
}
			 ...
			 alignment:(int)alignment;
{
    ...
    if (label >= 1 && label <= 7)
    {
        ...
        labelColor = [NTLabelColorMgrShared color:label];
    }

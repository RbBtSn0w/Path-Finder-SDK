...
@interface NTVolumeSpec ()
@property (nonatomic, retain) NSNumber *availableCapacity;
@property (nonatomic, retain) NSNumber *totalCapacity;
@property (nonatomic, retain) NSString *localizedFormat;
...
- (NSString*)makeVolumeUniqueID;

- (void)initVolumeInfo;
- (void)initVolumeParams;
- (void)preloadIfThread;
...
- (void)setMountPath:(NSString *)theMountPath;
...
@synthesize availableCapacity;
@synthesize totalCapacity;
@synthesize localizedFormat, volumeUniqueID;
...
+ (NTVolumeSpec*)volumeWithMountPoint:(NTFileDesc*)desc;
{
	...
	result.volumeUniqueID = [result makeVolumeUniqueID];

	[result preloadIfThread];
...
- (void)dealloc
{
	...
	self.availableCapacity = nil;
	self.totalCapacity = nil;
	self.localizedFormat = nil;
...
{
	@synchronized(self) {
		if (!mMountPath && [self statfsPtr])
			[self setMountPath:[NSString stringWithFileSystemRepresentation:[self statfsPtr]->f_mntonname]];
	}
	
	return [[mMountPath retain] autorelease];
}
...
{	
	// returns hfs, not HFS+ etc
	// if (self.localizedFormat)
...
- (NSString*)makeVolumeUniqueID;
{
	...
	NSString* mediaUUID = nil;
	
	DASessionRef sessionRef = DASessionCreate(kCFAllocatorDefault);
	if (sessionRef)
	{
		NSString* theBSDName = [self diskIDString];
		if ([theBSDName length])
		{
			DADiskRef diskRef = DADiskCreateFromBSDName(kCFAllocatorDefault, sessionRef, [theBSDName UTF8String]);
			
			if (diskRef)
			{
				CFDictionaryRef ref = DADiskCopyDescription(diskRef);
				
				if (ref)
				{
					// volume UUID
					CFUUIDRef uuidRef = CFDictionaryGetValue(ref, kDADiskDescriptionVolumeUUIDKey);
					if (uuidRef)
					{
						// convert uuid to string so we can use it as a key in our dictionary
						CFStringRef stringRef = CFUUIDCreateString(kCFAllocatorDefault, uuidRef);
						if (stringRef)
						{	
							volumeUUID = [NSString stringWithString:(NSString*)stringRef];
					...
					// media UUID
					uuidRef = CFDictionaryGetValue(ref, kDADiskDescriptionMediaUUIDKey);
					if (uuidRef)
					{
						// convert uuid to string so we can use it as a key in our dictionary
						CFStringRef stringRef = CFUUIDCreateString(kCFAllocatorDefault, uuidRef);
						if (stringRef)
						{	
							mediaUUID = [NSString stringWithString:(NSString*)stringRef];
	...
	// not sure if possible, but some code relies on this returning nil on failure, so being safe
	if (volumeUUID && ![volumeUUID length])
		volumeUUID = nil;
	if (mediaUUID && ![mediaUUID length])
		mediaUUID = nil;
	
	if (mediaUUID || volumeUUID)
		return [NSString stringWithFormat:@"%@/%@", volumeUUID, mediaUUID];
...
- (void)initVolumeInfo;
{
	@synchronized(self) {
		if (!mv_bools.volumeInfo_initialized)
		{
			...
			if (err)
				NSLog(@"Error: %d in %@ : %@", err, NSStringFromSelector(_cmd), [[self mountPointDesc] path]);
			
			// this shit doesn't work
			// fill in some stuff using URL api.  
			//   NSURL *theURL = [[self mountPointDesc] URL];
			//	if (theURL)
			//	{
			//		self.availableCapacity = [theURL resourceForKey:NSURLVolumeAvailableCapacityKey];
			//		self.totalCapacity = [theURL resourceForKey:NSURLVolumeTotalCapacityKey];
			//		self.localizedFormat = [theURL resourceForKey:NSURLVolumeLocalizedFormatDescriptionKey];
			//	}
...
- (void)setMountPath:(NSString *)theMountPath;
{
    if (mMountPath != theMountPath) {
        [mMountPath release];
        mMountPath = [theMountPath retain];
    }
}
...
- (void)preloadIfThread;
{
	// if we are in a thread, go ahead and load up the info.  If on main thread, there is no benefit
...
- (UInt64)totalBytes;
{	
	if (self.totalCapacity)
		return [self.totalCapacity unsignedLongLongValue];
...
{	
	if (self.availableCapacity)
		return [self.availableCapacity unsignedLongLongValue];

...
@interface NTFileListMonitor ()
@property (nonatomic, retain) NSDictionary *rsrcFileTable;
@property (nonatomic, assign) id<NTFileListMonitorDelegate> delegate;  // not retained
@end
...
@synthesize descs, delegate, rsrcFileTable;
...
- (void)scanList;
{
	[self setThreadRunner:[NTFileListMonitorThread thread:[self descs] rsrcFileTable:self.rsrcFileTable delegate:self]];
}
...
- (void)threadRunner_complete:(NTThreadRunner*)threadRunner;
{
	if (threadRunner == [self threadRunner])
	{
		...
		self.rsrcFileTable = param.rsrcFileTable;

...
@interface NTResourceMgr ()
@property (nonatomic, assign) BOOL isValid;
@property (nonatomic, assign) BOOL useDataFork;
@property (nonatomic, retain) NSDictionary *resourceMapDictionary;
...
@synthesize isValid;
@synthesize useDataFork;
@synthesize resourceMapDictionary;
@synthesize fileContents;
@synthesize desc;

+ (id)mgrWithDesc:(NTFileDesc*)desc useDataFork:(BOOL)useDataFork;
{
    NTResourceMgr* mgr = [[NTResourceMgr alloc] init];

	mgr.desc = desc;
    mgr.useDataFork = useDataFork;
    mgr.isValid = YES;
...
- (void)dealloc
{
    self.resourceMapDictionary = nil;
...
- (NSDictionary*)resourceMap;
{
    if (!self.resourceMapDictionary && self.isValid)
    {
        ...
        if (fileData)
        {
            ...
            if (map)
                self.resourceMapDictionary = [NSDictionary dictionaryWithDictionary:map];
        }
    }

    return self.resourceMapDictionary;
}
...
- (NSData*)fileData;
{
    if (!self.fileContents)
    {
        ...
        if ([self.desc isFile])
        {
			NSData* data = [NSData resourceData:self.desc rsrcFork:!self.useDataFork];

...
{
	return [[self copyDirectoryContents:visibleOnly resolveIfAlias:resolveIfAlias infoBitmap:kDefaultCatalogInfoBitmap] autorelease];
}

// *** caller must release, not autoreleased
- (NSArray*)copyDirectoryContents:(BOOL)visibleOnly resolveIfAlias:(BOOL)resolveIfAlias infoBitmap:(FSCatalogInfoBitmap)infoBitmap;
{
    NSArray* result=nil;
...
- (BOOL)hasDirectoryContents:(BOOL)visibleOnly;
{
	...
	BOOL result=NO;
    if (visibleOnly)
		[cachedData hasVisibleDirectoryContents_initialized:&result];
	else
		[cachedData hasDirectoryContents_initialized:&result];
...
{
	BOOL result;
	if (![cachedData hasDirectoryContents_initialized:&result])
	{
		...
		[cachedData setHasDirectoryContents:hasDirectoryContents];
		[cachedData setHasVisibleDirectoryContents:hasVisibleDirectoryContents];
	}
}
...
- (void)flushCatalogInfoHack;
{
	...
	OSStatus outStatus = FSOpenIterator( [self FSRefPtr], kFSIterateFlat, &iterator );
	if (outStatus == noErr)
	{
		ItemCount actualCount;

...
+ (NSMutableArray*)freshDescs:(NSArray*)descs;
{
	NSMutableArray* result = [NSMutableArray arrayWithCapacity:[descs count]];
	
    for (NTFileDesc* desc in descs)
	{
		desc = [desc freshDesc];
...
+ (NSString *)relativePath:(NTFileDesc*)theDesc;
{
	NSString* path = [theDesc path];
	NTVolume *volume = [theDesc volume];
	NSString* mountPoint = [[volume mountPoint] path];
	BOOL caseSensitive = [volume caseSensitive];
	
	// added to fix FSEvents bug?
	path = [path normalizedPath];
	
	if (mountPoint.length > 1) // not "/"
		path = [path stringByDeletingPrefix:mountPoint caseSensitive:caseSensitive];
...
- (NSMenu*)pathMenu:(SEL)action target:(id)target fontSize:(int)fontSize;
{
	...
	{		
		...
		[menuItem setImage:[desc iconImage:(fontSize == kSmallMenuFontSize) ? kSmallMenuIconSize : kDefaultMenuIconSize]];
...
+ (void)deleteResourceFork:(NTFileDesc*)desc;
{
    ...
    if (err == noErr)
		FSDeleteFork([desc FSRefPtr], forkName.length, forkName.unicode);
}
...
+ (void)setResourceFork:(NTFileDesc*)desc length:(UInt64)length;
{
    ...
    if (err == noErr)
    {
        ...
        if (err == noErr)
        {
            FSSetForkSize(forkRefNum, fsFromStart, length);
...
+ (void)copy:(NTFileDesc*)srcDesc fromDataFork:(BOOL)fromDataFork to:(NTFileDesc*)destDesc toDataFork:(BOOL)toDataFork;
{
    ...
    err1 = FSGetResourceForkName(&rsrcForkName);
    err2 = FSGetDataForkName(&dataForkName);
...
+ (BOOL)isInsidePackage:(NTFileDesc*)theDesc;
{
	do {
		theDesc = [theDesc parentDesc];
		
		if (theDesc && [theDesc isPackage])
			return YES;
...
@implementation NSArray (NTFileDescArray)

- (BOOL)arrayContainsAlias;
{	
    for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isAlias])
            return YES;
    }
...
- (BOOL)arrayContainsEjectable;
{	
    for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isEjectable])
            return YES;
    }
...
- (BOOL)arrayContainsPackage:(BOOL)includeDirectoriesWithExtensions;  // a unix rtfd folder?
{
    for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isPackage])
            return YES;
    }
	
	// folder.rtfd? without package bit set?
	if (includeDirectoriesWithExtensions)
	{
		for (NTFileDesc* theDesc in self)
		{
			if ([theDesc isDirectory])
			{
				if ([[theDesc extension] length])
					return YES;
			}
		}
	}
...
- (BOOL)arrayContainsFile:(BOOL)packagesOK;
{
	for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isFile])
        ...
        else if (packagesOK && [theDesc isPackage])
            return YES;
    }
...
- (BOOL)arrayContainsDirectory:(BOOL)packagesOK;
{	
	for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isDirectory])
...
- (BOOL)arrayContainsItemWithCustomIcon;
{	
	for (NTFileDesc* theDesc in self)
	{
		// *** if on a network, just bail out, too slow for the main thread
		if ([theDesc isNetwork])
			return NO;
		
		if ([theDesc isFile])
		{
			// checking the resource fork size is faster than trying to open the resource fork and checking for a resource
			if ([theDesc rsrcForkSize] > 0)
			{
				if ([NTIconFamily hasCustomIconForFile:theDesc])
					return YES;
			}
		}
		else // is directory
		{
			if ([NTIconFamily hasCustomIconForDirectory:theDesc])
				return YES;
		}
	}
...
- (BOOL)arrayContainsImageWithoutACustomIcon;
{	
	for (NTFileDesc* theDesc in self)
	{
		// *** if on a network, just bail out, too slow for the main thread
		...
		if ([theDesc isFile] && [[theDesc typeIdentifier] isImage])
		{
			// checking the resource fork size is faster than trying to open the resource fork and checking for a resource
			if ([theDesc rsrcForkSize] > 0)
			{
...
- (BOOL)arrayContainsImage;
{	
	for (NTFileDesc* theDesc in self)
    {
        if ([[theDesc typeIdentifier] isImage])
            return YES;
    }
...
- (BOOL)arrayContainsImagesOrMedia;
{	
	for (NTFileDesc* theDesc in self)
    {
		NTFileTypeIdentifier *type = [theDesc typeIdentifier];
		
        if ([type isImageOrMedia])
            return YES;
    }
...
- (BOOL)arrayContainsWritableItem;
{	
	for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isWritable])
            return YES;
    }
...
- (BOOL)arrayContainsModifiableItem;
{	
	for (NTFileDesc* theDesc in self)
	{
        if (![theDesc isReadOnly])
            return YES;
    }
...
- (BOOL)arrayContainsMovableItem;
{	
	for (NTFileDesc* theDesc in self)
    {
        if ([theDesc isMovable])
            return YES;
    }

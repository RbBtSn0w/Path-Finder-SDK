...
- (void)mouseEntered:(NSEvent*)event
{
	...
	[[self menuItemView] updateLabelText:[NTLabelColorMgrShared label:[NTLabelColorMgrShared labelAtIndex:self.labelIndex]]];
...
- (void)mouseUp:(NSEvent*)event
{
	...
	if (NSPointInRect(mousePoint, [self bounds]))
		[NSApp sendAction:[[self enclosingMenuItem] action] to:[[self enclosingMenuItem] target] from:[NSNumber numberWithInt:[NTLabelColorMgrShared labelAtIndex:self.labelIndex]]];
...
{	
	...
	int label = [NTLabelColorMgrShared labelAtIndex:self.labelIndex];
	
	NSColor *color = [NTLabelColorMgrShared color:label];
	if (color)
		[color set];
	else
	...
	if (!color)
	{
		...
		[[self sharedStopImage:imageRect.size] drawInRect:imageRect fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1];
	}

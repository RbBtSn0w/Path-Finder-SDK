...
@interface NTITunesKQueueEvent: NSObject
{
	int mv_fd;
...
+ (NTITunesKQueueEvent*)eventWithFD:(int)fd fflags:(unsigned int)fflags uniqueID:(unsigned int)uniqueID;
...
- (void)notifyObserverWithResponse:(NTITunesKQueueEvent*)response;
...
- (NSString *)databasePath
{
	if (!mDatabasePath)
	{
		...
		if ([xmlArray count])
		{
			NSString* thePath = [xmlArray objectAtIndex:0];
			if (thePath)
			{
			...
			switch (event.filter)
			{
				...
				{					
					...
					[self performSelectorOnMainThread:@selector(notifyObserverWithResponse:)
										   withObject:[NTITunesKQueueEvent eventWithFD:fd fflags:event.fflags uniqueID:uniqueID] 
										waitUntilDone:NO];
				}
					break;
			}
...
@implementation NTITunesKQueueEvent
...
+ (NTITunesKQueueEvent*)eventWithFD:(int)fd fflags:(unsigned int)fflags uniqueID:(unsigned int)uniqueID;
{
	NTITunesKQueueEvent* result = [[NTITunesKQueueEvent alloc] initWithFD:fd fflags:fflags uniqueID:uniqueID];

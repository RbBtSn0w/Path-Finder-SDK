...
{
	[[self displayMgr] appendToToolLog:[NSString stringWithFormat:@"%@ %@", self.toolPathMgr.SVNTool, [args description]]];
	
	[[self launchedTools] addObject:[[self host] runTool:self.toolPathMgr.SVNTool
	...
	if ([context isKindOfClass:[NSString class]])
	{
		...
		else if ([context isEqualToString:@"askUserForOtherCommand"])
		{
			if ([result length] && [self directory])
			{
				...
				result = [NSString stringWithFormat:@"%@ %@", self.toolPathMgr.SVNTool, result];

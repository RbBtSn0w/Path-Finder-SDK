...
Boolean sendRevealEvent(const FSRef * fsRef)
{
    ...
    AECreateDesc(typeApplSignature, &appSignature,
                       sizeof(appSignature),
                       &targetAddress);
    ...
    FSNewAliasMinimal(fsRef, &alias);
    HLock((Handle) alias);
    AEPutParamPtr(&ae, keyDirectObject, typeAlias, *alias,
        ...
        if (err == noErr)
        {
            ...
            if (err == noErr && !(info.processMode & modeOnlyBackground))
            {
                if (info.processSignature == kPathFinderSignature)
                {
                    ...
                    if (outRef)
                        GetProcessBundleLocation(&PSN,outRef);
